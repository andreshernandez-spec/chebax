...
betaincinv and stdtr/stdtrit need (a, b) inside the betainc table domain
[0.1, 10]. stdtr/stdtrit run on their own 2-D slice tables of the same
kernel at a = 1/2 (b-panels to 100), so they cover nu in [0.2, 200]
without the 3-D tensor. gammaincinv has no table domain: its Newton
...
def _gammaincinv_solve(a, pc, use_recipe):
    ...
    python bool (each lax.cond branch traces one): True takes both logs
    from the chebax gammainc tables, reconstructed once and closed over,
    so each step is a fixed-degree polynomial; False uses jax's gammainc
    and gammaincc (any a, but a whole-array while_loop per call).
...
def gammaincinv(a, p):
    ...
    for every p, endpoints included. For a inside [0.1, 10] the Newton
    residual runs on the chebax gammainc tables (fixed-degree polynomials);
    outside the box it falls back to jax's gammainc, so the domain stays
    all of a > 0. The upper half is solved against Q, so p within rounding
    of 1 still resolves (P(a, x) - p there has no digits left).
    Quantiles below the smallest normal float of the canonical dtype return
...
def chi2inv(k, p):
    ...
    gives nan for every p. Endpoints follow gammaincinv (p = 0 -> 0,
    p = 1 -> inf)."""
