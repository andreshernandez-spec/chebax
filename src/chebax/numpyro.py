...
(concentration1, concentration0) in [0.1, 10]^2; TruncatedStudentT needs
df in [0.2, 200]; TruncatedGamma needs only concentration > 0, its
normalizer falling back to jax's own gammainc pair outside chebax's
[0.1, 10] box (which costs the deep upper tail there, since those
underflow). Rates, bounds and data are unrestricted.
...
def _qinv_solve(a, sc, use_recipe):
    ...
    seam = _gt.XS if use_recipe else 1.0 + a
...
class TruncatedBeta(_Truncated):
    ...
    [0.1, 10]^2 (tables back the normalizer's gradient and the inverse
